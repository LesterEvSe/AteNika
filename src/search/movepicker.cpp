    ...
    for (uint8_t i = 0; i < m_move_list.size(); ++i) {
        ...
        switch (m_move_list[i].get_flag()) {
            case Move::CAPTURE_PROMOTION:
                score = MvvLva::PROMOTION_BONUS + MvvLva::CAPTURE_BONUS +
                        MvvLva::mvv_lva[m_move_list[i].get_captured_piece()]
                                       [m_move_list[i].get_move_piece()];
                break;
            case Move::EN_PASSANT:
            case Move::CAPTURE:
                score = MvvLva::CAPTURE_BONUS + MvvLva::mvv_lva[m_move_list[i].get_captured_piece()]
                                                               [m_move_list[i].get_move_piece()];
                break;
            ...
            default:
                if (m_move_list[i] == order_info.get_killer1())
                    ...
                    score = order_info.get_history(m_move_list[i].get_from_cell(),
