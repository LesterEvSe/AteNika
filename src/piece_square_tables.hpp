...
namespace hidden {
    extern int32_t piece_sq_tables[COLOR_SIZE][PHASES][PIECE_SIZE][64];

    // PT - Piece Table
    int32_t *create_pawns_pt   (GamePhase phase);
    int32_t *create_knights_pt (GamePhase phase);
    int32_t *create_bishops_pt (GamePhase phase);
    int32_t *create_rooks_pt   (GamePhase phase);
    int32_t *create_queens_pt  (GamePhase phase);
    int32_t *create_king_pt    (GamePhase phase);
}; // hidden
    void init();
