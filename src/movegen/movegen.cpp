...
void Movegen::gen_pawn_promotion(uint8_t from, uint8_t to, Move::Flag flag,
...
void Movegen::gen_white_left_pawn_captures() {
    ...
    if (en_passant_cell && (ONE << en_passant_cell) & attacks)
        m_moves.emplace_back(
            Move(en_passant_cell - 7, en_passant_cell, PAWN, Move::EN_PASSANT, PAWN));
...
void Movegen::gen_black_left_pawn_captures() {
    ...
    if (en_passant_cell && (ONE << en_passant_cell) & attacks)
        m_moves.emplace_back(
            Move(en_passant_cell + 9, en_passant_cell, PAWN, Move::EN_PASSANT, PAWN));
