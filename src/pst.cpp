...
void PieceTables::init() {
    ...
    for (Color color : {WHITE, BLACK})
        for (uint8_t i = 0; i < 64; ++i) {
            ...
            piece_sq_tables[color][OPENING][PAWN][j]   = pawns[OPENING][i];
            piece_sq_tables[color][OPENING][KNIGHT][j] = knight[OPENING][i];
            piece_sq_tables[color][OPENING][BISHOP][j] = bishop[OPENING][i];
            piece_sq_tables[color][OPENING][ROOK][j]   = rook[OPENING][i];
            piece_sq_tables[color][OPENING][QUEEN][j]  = queen[OPENING][i];
            piece_sq_tables[color][OPENING][KING][j]   = king[OPENING][i];
            piece_sq_tables[color][ENDGAME][KING][j]   = king[ENDGAME][i];
        }
}
