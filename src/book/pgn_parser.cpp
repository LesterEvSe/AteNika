...
void PGNParser::detail::first_processing(const std::string &path) {
    ...
    auto condition = [](const std::string &str) { return str[0] == '3'; };
...
namespace {
    ...
    bool promotion_check(const std::string &com, const Move &move) {
        ...
        return com.substr(ind3, 2) == temp.substr(2, 2) && com[3 + ind3] == temp[4] &&
               (!ind3 || com[0] == temp[0]);
    }
    bool other_check(const std::string &command, const Move &move) {
        ...
        if (bool digit = isdigit(command[1]); command.size() == 4)
            return command[1] == temp[digit] && move.get_move_piece() == type &&
                   command.substr(2, 2) == last;
    ...
    while (std::getline(from, line)) {
        ...
        while (iss >> command) {
            ...
            if (i == moves.size()) {
                ...
                std::cerr << "Move: " << counter / 3 << ' '
                          << (counter % 3 == 2 ? "White" : "Black") << std::endl;
