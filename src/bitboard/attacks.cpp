...
bitboard Attacks::detail::_rook_mask[64];
bitboard Attacks::detail::_rook_attacks[64][4096];

bitboard Attacks::detail::_bishop_mask[64];
bitboard Attacks::detail::_bishop_attacks[64][512];

bitboard Attacks::detail::_pawn_attacks[COLOR_SIZE][64];
bitboard Attacks::detail::_knight_attacks[64];
bitboard Attacks::detail::_king_attacks[64];

void Attacks::init() {
  detail::_init_rook_mask();
  detail::_init_bishop_mask();
  detail::_init_rook_attacks();   // using _rook_mask
  detail::_init_bishop_attacks(); // using _bishop_mask

  detail::_init_pawn_attacks();
  detail::_init_knight_attacks();
  detail::_init_king_attacks();
}

// convert from mask to blockers pieces
bitboard Attacks::detail::_get_blockers(uint16_t index, bitboard mask) {
...
bitboard Attacks::detail::_calculate_rook_attacks(uint8_t cell, bitboard blockers) {
...
bitboard Attacks::detail::_calculate_bishop_attacks(uint8_t cell, bitboard blockers) {
  bitboard temp, attack = ZERO;
...
void Attacks::detail::_init_rook_mask() {
...
void Attacks::detail::_init_bishop_mask() {
...
void Attacks::detail::_init_rook_attacks() {
  for (uint8_t i = 0; i < 64; ++i)
...
void Attacks::detail::_init_bishop_attacks() {
  for (uint8_t i = 0; i < 64; ++i)
...
void Attacks::detail::_init_pawn_attacks() {
...
void Attacks::detail::_init_knight_attacks() {
  for (uint8_t i = 0; i < 64; ++i) {
    bitboard piece = ONE << i;
    ...
    bitboard lr2 = ((piece >> 2) & ~(FILE_G | FILE_H)) | ((piece << 2) & ~(FILE_A | FILE_B));
    _knight_attacks[i] = lr1 << 16 | lr1 >> 16 | lr2 << 8 | lr2 >> 8;
  }
}

void Attacks::detail::_init_king_attacks() {
  for (uint8_t i = 0; i < 64; ++i) {
    bitboard piece = ONE << i;

    _king_attacks[i] = (((piece << 7) | (piece >> 1) | (piece >> 9)) & ~FILE_H) | (piece << 8) |
                       (piece >> 8) | (((piece << 9) | (piece << 1) | (piece >> 7)) & ~FILE_A);
  }
}

bitboard Attacks::get_rook_attacks(uint8_t cell, bitboard blockers) {
  blockers &= detail::_rook_mask[cell];
  uint64_t key = (blockers * detail::_rook_magics[cell]) >> (64 - detail::_rook_bits[cell]);
  return detail::_rook_attacks[cell][key];
}

bitboard Attacks::get_bishop_attacks(uint8_t cell, bitboard blockers) {
  blockers &= detail::_bishop_mask[cell];
  uint64_t key = (blockers * detail::_bishop_magics[cell]) >> (64 - detail::_bishop_bits[cell]);
  return detail::_bishop_attacks[cell][key];
}
...
bitboard Attacks::get_pawn_attacks(Color color, uint8_t cell) {
  return detail::_pawn_attacks[color][cell];
}
bitboard Attacks::get_knight_attacks(uint8_t cell) { return detail::_knight_attacks[cell]; }
bitboard Attacks::get_king_attacks(uint8_t cell) { return detail::_king_attacks[cell]; }