...
bool Transposition::in_table(const ZobristHash &hash) {
...
void Transposition::set(const ZobristHash &hash, const TTEntry &entry) {
    hidden::_tt[(bits96)hash.get_hash()] = entry;
}