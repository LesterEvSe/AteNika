...
private:
    ...
    // 4096 = 2^(max element from RookBits array)
    static bitboard RookMask[64];
    static bitboard RookAttack[64][4096];

    // 1024. Why not 512? 2^9
    static bitboard BishopMask[64];
    static bitboard BishopAttack[64][1024];


    static uint64_t get_blockers(uint16_t index, uint64_t mask);
    static void init_rook_mask();
    static void init_rook_table();
    static bitboard calculate_rook_attacks(uint8_t ind, uint64_t blockers);

    static void init_bishop_mask();
    static void init_bishop_table();
    static bitboard calculate_bishop_attacks(uint8_t ind, uint64_t blockers);
